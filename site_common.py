...
#
# Входа через Google больше нет — только почта и пароль. Он так и не
# заработал («The requested action is invalid»), а настраивать его надо
# в трёх местах сразу: Firebase, Google Cloud и экран согласия. Кнопка,
# которая не работает, хуже, чем её отсутствие.
...
// Коды Firebase человеческим языком.
var AUTH_ERRORS = {
...
// действительно произошёл, — после формы.
function greet(user) {
