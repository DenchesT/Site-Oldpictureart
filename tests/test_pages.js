...
(async () => {
  ...
  // Число берём из базы, а не вписываем: собрание пополняется, и жёсткая
  // цифра ломала бы проверку после каждого нового поста, ничего не находя.
  ok('всего работ посчитано верно', st.total === String(_meta.length),
    `${st.total} на странице, ${_meta.length} в базе`);
